    ...
    public function ticket_checkout()
    {
        ...
        try {
            ...
            if (!empty($_POST['stripeToken'])) {
                ...
                $time = time();
                //item information
                $itemName = ($setting->ticket_title) ? $setting->ticket_title : "Ticket Charges";
                // $itemNumber = ($setting->sub_interval) ? $setting->sub_interval : 'year';
                $itemPrice = $setting->ticket_price;
                $currency = ($setting->ticket_currency) ? $setting->ticket_currency : "usd";
                $orderID = $time;
                //charge a credit or a debit card
                $charge = \Stripe\Charge::create(array(
                    'customer' => $customer->id,
                ...
                if ($chargeJson['amount_refunded'] == 0 && empty($chargeJson['failure_code']) && $chargeJson['paid'] == 1 && $chargeJson['captured'] == 1) {
                    ...
                    if ($this->db->insert('ticket_payments', $dataDB)) {
                        if ($this->db->insert_id() && $status == 'succeeded') {
